...
export class JwtService {
  ...
  async createAccessToken(userId: string, deviceId: string, role: string): Promise<string> {
    await this.redisService.set(deviceId, userId, this.configService.get('MAX_AGE_ACCESS_TOKEN_FOR_REDIS'));
    return jwt.sign({ userId, deviceId, role }, this.configService.get('JWT_SECRET')!, {
